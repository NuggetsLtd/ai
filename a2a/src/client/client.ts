...
import { verifyClient } from "../nuggets/index.js";
import {
  // Core types
  AgentCard,
...
export class A2AClient {
  ...
  async authenticateAgentWithNuggets() {
    const authenticationUrl = `${this.baseUrl}/authenticate`;
    const response = await this.fetchImpl(authenticationUrl);
    ...
    if (response.ok) {
      ...
      await verifyClient({
        clientId: decoded.payload.sub as string,
        token: json.token,
      });
